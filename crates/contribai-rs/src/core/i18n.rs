...
static TRANSLATIONS: LazyLock<HashMap<(Locale, MsgKey), &'static str>> = LazyLock::new(|| {
    ...
    // English (default)
    m.insert((Locale::En, MsgKey::PipelineStart), "🚀 Starting ContribAI pipeline");
    m.insert((Locale::En, MsgKey::DiscoveringRepos), "🔍 Discovering repositories...");
    m.insert((Locale::En, MsgKey::AnalyzingRepo), "📦 Analyzing repository");
    ...
    m.insert((Locale::En, MsgKey::CircuitBreakerOpen), "🔴 Circuit breaker is open");
    m.insert((Locale::En, MsgKey::CacheHit), "💾 Cache hit");
    // Vietnamese
    m.insert((Locale::Vi, MsgKey::PipelineStart), "🚀 Bắt đầu ContribAI pipeline");
    m.insert((Locale::Vi, MsgKey::DiscoveringRepos), "🔍 Đang tìm repositories...");
    m.insert((Locale::Vi, MsgKey::AnalyzingRepo), "📦 Đang phân tích repository");
    ...
    m.insert((Locale::Vi, MsgKey::CircuitBreakerOpen), "🔴 Circuit breaker đang mở");
    m.insert((Locale::Vi, MsgKey::CacheHit), "💾 Cache hit");
    // Japanese
    m.insert((Locale::Ja, MsgKey::PipelineStart), "🚀 ContribAIパイプラインを開始");
    m.insert((Locale::Ja, MsgKey::DiscoveringRepos), "🔍 リポジトリを検索中...");
    m.insert((Locale::Ja, MsgKey::AnalyzingRepo), "📦 リポジトリを分析中");
    m.insert((Locale::Ja, MsgKey::NoFindings), "✅ 問題は見つかりませんでした");
    ...
    m.insert((Locale::Ja, MsgKey::CircuitBreakerOpen), "🔴 サーキットブレーカーが開いています");
    m.insert((Locale::Ja, MsgKey::CacheHit), "💾 キャッシュヒット");
    // Chinese (Simplified)
    m.insert((Locale::ZhCn, MsgKey::PipelineStart), "🚀 启动 ContribAI 管道");
    m.insert((Locale::ZhCn, MsgKey::DiscoveringRepos), "🔍 正在搜索仓库...");
    ...
    m.insert((Locale::ZhCn, MsgKey::CircuitBreakerOpen), "🔴 断路器已打开");
...
pub fn t(locale: Locale, key: MsgKey) -> &'static str {
    TRANSLATIONS
        .get(&(locale, key))
        .copied()
        .unwrap_or_else(|| TRANSLATIONS.get(&(Locale::En, key)).copied().unwrap_or("unknown"))
}
...
mod tests {
    ...
    fn test_translate_english() {
        assert_eq!(t(Locale::En, MsgKey::PipelineStart), "🚀 Starting ContribAI pipeline");
        assert_eq!(t(Locale::En, MsgKey::NoFindings), "✅ No findings");
    }

    #[test]
    fn test_translate_vietnamese() {
        assert_eq!(t(Locale::Vi, MsgKey::PipelineStart), "🚀 Bắt đầu ContribAI pipeline");
        assert_eq!(t(Locale::Vi, MsgKey::NoFindings), "✅ Không tìm thấy vấn đề");
    }

    #[test]
    fn test_translate_japanese() {
        assert_eq!(t(Locale::Ja, MsgKey::PipelineStart), "🚀 ContribAIパイプラインを開始");
        assert_eq!(t(Locale::Ja, MsgKey::NoFindings), "✅ 問題は見つかりませんでした");
    }

    #[test]
    fn test_translate_chinese() {
        assert_eq!(t(Locale::ZhCn, MsgKey::PipelineStart), "🚀 启动 ContribAI 管道");
    ...
    fn test_current_locale_default() {
        ...
        assert!(matches!(locale, Locale::En | Locale::Vi | Locale::Ja | Locale::ZhCn));
    }
